...
SignIn(onTap: () {
  if (state.isValid) {
    ...
    // Construir un mensaje de error detallado basado en los errores específicos
    List<String> errorMessages = [];

    if (state.errors['name'] != null && state.errors['name']!.isNotEmpty) {
      errorMessages.add('Nombre: ${state.errors['name']}');
    }
    if (state.errors['lastname'] != null && state.errors['lastname']!.isNotEmpty) {
      errorMessages.add('Apellido: ${state.errors['lastname']}');
    }
    if (state.errors['email'] != null && state.errors['email']!.isNotEmpty) {
      errorMessages.add('Correo: ${state.errors['email']}');
    }
    if (state.errors['rfc'] != null && state.errors['rfc']!.isNotEmpty) {
      errorMessages.add('RFC: ${state.errors['rfc']}');
    }
    if (state.errors['phone'] != null && state.errors['phone']!.isNotEmpty) {
      errorMessages.add('Teléfono: ${state.errors['phone']}');
    }
    if (state.errors['password'] != null && state.errors['password']!.isNotEmpty) {
      errorMessages.add('Contraseña: ${state.errors['password']}');
    }
    if (state.errors['id_banck'] != null && state.errors['id_banck']!.isNotEmpty) {
      errorMessages.add('ID Banco: ${state.errors['id_banck']}');
    }

    // Unir los mensajes de error en una sola cadena, separada por saltos de línea
    final errorMessage = errorMessages.join('\n');

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(errorMessage.isNotEmpty ? errorMessage : 'Por favor complete todos los campos correctamente'),
